// forked from design-sync lib/source-kit.mjs — snake_case section dirs (contact_form,
// rich_text) must count as their component's own dir (ContactForm, RichTextSection)
// in the group filter, so the group falls through to the doc frontmatter category.
// Normalized dir must equal or prefix the component name (rich_text → RichTextSection).
    ...
    for (const c of components) {
      ...
      c.group = slug(
        slash(relative(srcRoot, dirname(hit)))
          .split('/')
          .filter((s) => s && !c.name.toLowerCase().startsWith(s.toLowerCase().replace(/[^a-z0-9]/g, '')) && !GENERIC_DIR.has(s.toLowerCase()))
          .at(-1)
